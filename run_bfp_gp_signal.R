## Agatha BFP with GP noise model (noise.only=FALSE): fit sinusoid + GP simultaneously.
## GP kernel: SHO with fixed Prot (rotation period), free sigmaGP and logtauGP.
## This is the Goldilocks approach from Feng+2016 MNRAS 461, 2440.
##
## Usage: Rscript run_bfp_gp_signal.R <input.vels> <outdir> <ofac> [Prot]
...
# Source periodoframe.R LAST so its sho.term (S0,Q,w0) overrides mcmc_func's (S0,logQ,logProt)
...
# Fix logProt at rotation period; free sigmaGP and logtauGP
gp.par <- c(NA, log(Prot), NA)
cat(sprintf("%s  N=%d  RMS=%.3f  Prot=%.1fd (fixed)  ofac=%.2f\n",
            target, N, sqrt(mean(y^2)), Prot, ofac))

fmin <- 1/max(diff(range(t)), 5000); fmax <- 1/1.1

# BFP with GP=TRUE, noise.only=FALSE: fits sinusoid + GP noise at each trial period
cat("Running BFP(GP=TRUE, noise.only=FALSE)...\n")
t0 <- Sys.time()
per <- tryCatch(
...
cat(sprintf("\n  BFP+GP(signal): time=%.1fs  max(lnBF)=%.2f at P=%.4fd\n", dt, max(lnbf), P[which.max(lnbf)]))
...
# Save
tag <- sprintf("GP_Prot%.0f_signal", Prot)
...
sink(f_sum)
cat("# Agatha BFP + GP(signal) for", target, "\n")
cat("# GP: SHO with fixed Prot=", Prot, "d, free sigmaGP & logtauGP\n")
cat("# noise.only=FALSE: sinusoid + GP fitted simultaneously\n")
...
plot(P[isort], lnbf[isort], type = "l", log = "x", lwd = 0.5,
     ...
     main = sprintf("%s  BFP+GP(signal) Prot=%.0fd  max lnBF=%.2f at P=%.2fd",
                    target, Prot, max(lnbf), P[which.max(lnbf)]))
