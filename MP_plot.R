...
###plot
    ...
    if(exists('idata')){
        for(k in 1:Ntarget){
            ...
            if(k==1){
                if(j==1){
                    plot(t1,y1,ylab=ylab,xaxt='n',pch=20,cex=0.5,xlim=xlim,ylim=range(y),col=cols[1])
                }else{
                    plot(t1,y1,ylab=ylab,xaxt='n',yaxt='n',pch=20,cex=0.5,xlim=xlim,ylim=range(y),col=cols[1])
                }
            }else{
                points(t1,y1,col=cols[k],pch=20,cex=0.5)
            }
...
#        if(class(p)=='try-error')
        mtext(text='Time [JD]',side=1,outer=TRUE,line=2.2,cex=0.9*size)
