...
if(length(args)>0){
    ...
    input.file <- if(length(args) >= 7) as.character(args[7]) else NULL
}else{
    np <- 2
    NI <- 3
    Nma <- 0
...
if(!is.null(input.file)){
    ...
    target <- tools::file_path_sans_ext(basename(f))
}else if(grepl('.rdb',f)){
...
####set up
########################
if(!exists('target')) target <- gsub('_.+','',f)
#update <- FALSE
rescale <- FALSE#TRUE
...
t1 <- proc.time()
cat('fmax=',fmax,'\n')
bf <- BFP(tab[,1],tab[,2],tab[,3],Nma=Nma,Indices=Indices,ofac=ofac,model.type=model.type,fmax=fmax,progress=FALSE)
t2 <- proc.time()
...
#######make MLP 
t1 <- proc.time()
ml <- MLP(tab[,1],tab[,2],tab[,3],Nma=Nma,ofac=ofac,mar.type='part',model.type=model.type,fmax=fmax,opt.par=NULL,Indices=Indices,MLP.type=MLP.type)
t2 <- proc.time()
