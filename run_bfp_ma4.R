## Run Agatha BFP with MA(4) noise model (Nma=4, Nar=0) on alpha Cen residuals.
## Usage: Rscript run_bfp_ma4.R <input.vels> <outdir> <ofac>
...
ofac <- as.numeric(args[3]); if (is.na(ofac)) ofac <- 0.3
...
t0 <- Sys.time()
per <- BFP(t, y, ey, Nma = 4, Nar = 0, Indices = NULL, ofac = ofac,
...
cat(sprintf("  BFP+MA(4): time=%.1fs  max(lnBF)=%.2f at P=%.4fd  llmax=%.2f  ll0=%.2f  dll=%.2f\n",
...
f_asc <- file.path(outdir, paste0(target, "_BFP_MA4_periodogram.txt"))
write.table(cbind(Period.day = P[isort], lnBF = lnbf[isort]),
            file = f_asc, row.names = FALSE, quote = FALSE)

# Save summary
f_sum <- file.path(outdir, paste0(target, "_BFP_MA4_summary.txt"))
sink(f_sum)
cat("# Agatha BFP + MA(4) noise model for", target, "\n")
...
legend("topright", legend = c("ln(BF)=5 threshold", "annual aliases"),
       col = c("red", "grey60"), lty = c(2, 3), bty = "n", cex = 0.8)

# Short-period zoom 1.5-50 d
msel <- P > 1.5 & P < 50
plot(P[isort][msel[isort]], lnbf[isort][msel[isort]], type = "l", lwd = 0.7,
     xlab = "Period (d)", ylab = "ln(BF)",
     main = "Short-period zoom (1.5–50 d)")
abline(h = 5, lty = 2, col = "red"); abline(h = 0, lty = 3, col = "grey70")
magaxis(side = c(1, 2), tcl = -0.3)
dev.off()
cat("\nWrote", fpdf, "\n       ", f_sum, "\n       ", f_asc, "\n")
