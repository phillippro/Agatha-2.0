...
out_dir <- file.path("results", "HD361_combined_2signal_nightly_ma_modelselect_mcmc_kepler")
...
nightly_bin <- function(tab) {
  ...
  binned <- do.call(rbind, lapply(split(seq_len(nrow(tab)), night), function(ii) {
    w <- 1 / tab[[3]][ii]^2
    bj <- sum(tab[[1]][ii] * w) / sum(w)
    rv <- sum(tab[[2]][ii] * w) / sum(w)
    erv <- sqrt(1 / sum(w))
    c(BJD=bj, RV=rv, eRV=erv)
  }))
...
raw_counts <- sapply(tabs, nrow)
tabs <- lapply(tabs, nightly_bin)
binned_counts <- sapply(tabs, nrow)
...
noise_model_compare <- function(rows, max_ar=2, max_ma=2) {
  fits <- list()
  white <- NULL
  for (Nar0 in 0:max_ar) {
    for (Nma0 in 0:max_ma) {
      fit <- multiset_wls(t=rows$t, y=rows$y, dy=rows$dy, set.id=rows$set.id,
                          omega=NULL, trend=TRUE, Nar=Nar0, Nma=Nma0)
      if (Nma0 > 0) {
        for (iter in 1:3) {
          fit <- multiset_wls(t=rows$t, y=rows$y, dy=rows$dy, set.id=rows$set.id,
                              omega=NULL, trend=TRUE, Nar=Nar0, Nma=Nma0,
      ...
      n_decay <- length(levels(rows$set.id)) * ((Nar0 > 0) + (Nma0 > 0))
      k <- length(fit$coef) + n_decay
      bic <- -2 * fit$logL + k * log(nrow(rows))
      fits[[length(fits) + 1]] <- data.frame(
        Nar=Nar0, Nma=Nma0, model=noise_label(Nar0, Nma0),
        logL=fit$logL, npar=k, BIC=bic, stringsAsFactors=FALSE
      )
      if (Nar0 == 0 && Nma0 == 0) {
        white <- fits[[length(fits)]]
      }
    }
  }
  tab <- do.call(rbind, fits)
  tab$delta_BIC <- tab$BIC - min(tab$BIC)
  tab$lnBF_vs_white <- (tab$logL - white$logL) - 0.5 * (tab$npar - white$npar) * log(nrow(rows))
  tab <- tab[order(tab$BIC),]
  rownames(tab) <- NULL
  tab
}

lag_start <- function(per, id, prefix, order, fallback=0) {
  if (order <= 0) {
    return(numeric(0))
  }
  vals <- rep(fallback, order)
  for (lag in seq_len(order)) {
    nm <- paste0(prefix, "_", make.names(id), "_lag", lag)
    if (nm %in% names(per$par.opt)) {
      vals[lag] <- unname(per$par.opt[nm])
    }
  }
  vals
}

make_start <- function(periods, amps, phases, gammas, jitters, per=NULL,
                       beta0, trend0=0) {
  start <- c()
  ...
  for (k in seq_along(dataset)) {
    tmp <- c(gammas[k], jitters[k])
    names(tmp) <- c(paste0("b", k), paste0("s", k))
    if (Nar > 0) {
      ar <- if (is.null(per)) rep(0.1, Nar) else lag_start(per, dataset[k], "ar", Nar, fallback=0.1)
      ar <- pmax(-0.9, pmin(0.9, ar))
      names(ar) <- paste0("phi", k, seq_len(Nar))
      tmp <- c(tmp, ar)
      tmp <- c(tmp, setNames(beta0[k], paste0("alpha", k)))
    }
    if (Nma > 0) {
      ma <- if (is.null(per)) rep(0.1, Nma) else lag_start(per, dataset[k], "ma", Nma, fallback=0.1)
      ma <- pmax(-0.9, pmin(0.9, ma))
      names(ma) <- paste0("w", k, seq_len(Nma))
      tmp <- c(tmp, ma)
      tmp <- c(tmp, setNames(beta0[k], paste0("beta", k)))
    }
    start <- c(start, tmp)
  }
  start
...
configure_bounds <- function(startvalue, periods, amps) {
  ...
  for (k in seq_along(dataset)) {
    ...
    if (Nar > 0) {
      par.min[paste0("phi", k, seq_len(Nar))] <<- phi.min
      par.max[paste0("phi", k, seq_len(Nar))] <<- phi.max
      par.min[paste0("alpha", k)] <<- alpha.min
      par.max[paste0("alpha", k)] <<- alpha.max
    }
    if (Nma > 0) {
      par.min[paste0("w", k, seq_len(Nma))] <<- wmin
      par.max[paste0("w", k, seq_len(Nma))] <<- wmax
...
fit_rows_for <- function(best, label) {
  ...
  out_rows <- do.call(rbind, lapply(seq_along(dataset), function(k) {
    ...
    red <- if (nqp[2] > 0 || nqp[3] > 0) {
      arma(t=tab[[1]], ymodel=model, ydata=tab[[2]], pars=best,
           ind.set=k, p=nqp[3], q=nqp[2])$arma
    } else {
      rep(0, nrow(tab))
    }
    data.frame(dataset=id, BJD=tab[[1]], RV=tab[[2]], eRV=tab[[3]],
               kepler_signal=signal, model=model + red, red_noise=red,
               residual=tab[[2]] - model - red)
  }))
...
noise_comparison <- noise_model_compare(raw_rows, max_ar=0, max_ma=2)
write.csv(noise_comparison, file.path(out_dir, "noise_model_comparison.csv"),
          quote=FALSE, row.names=FALSE)
Nar <- noise_comparison$Nar[1]
Nma <- noise_comparison$Nma[1]
selected_noise <- noise_comparison$model[1]
...
cat("Combined HD361 two-signal Keplerian Agatha run with nightly binning\n")
cat("datasets:", paste(dataset, collapse=", "), "\n")
cat("raw counts:", paste(paste(dataset, raw_counts, sep="="), collapse=", "), "\n")
cat("nightly binned counts:", paste(paste(dataset, binned_counts, sep="="), collapse=", "), "\n")
cat("Ndata:", nrow(raw_rows), "Niter:", Niter, "selected noise:", selected_noise,
    "Nma:", Nma, "Nar:", Nar, "\n")
cat("Noise model comparison by BIC:\n")
...
start1 <- make_start(periods=per1$Popt, amps=max(sqrt(A1^2 + B1^2), 1e-3),
                     ...
                     jitters=jitters, per=per1, beta0=beta0,
                     trend0=unname(per1$par.opt["beta"]) * time.unit)
set.seed(361)
fit1_label <- paste0("one_signal_kepler_", selected_noise)
fit1 <- export_fit(fit_mcmc(start1, "one_signal"), fit1_label)
fit_rows1 <- fit_rows_for(fit1$best, fit1_label)
phase_plot(fit1$best, fit_rows1, fit1_label,
           file.path(out_dir, paste0("HD361_combined_", fit1_label, "_phase_fit.pdf")))
...
for (k in seq_along(dataset)) {
  nms <- c(paste0("b", k), paste0("s", k))
  if (Nar > 0) {
    nms <- c(nms, paste0("phi", k, seq_len(Nar)), paste0("alpha", k))
  }
  if (Nma > 0) {
    nms <- c(nms, paste0("w", k, seq_len(Nma)), paste0("beta", k))
  }
  tmp <- fit1$best[nms]
  noise2 <- c(noise2, tmp)
}
...
set.seed(362)
fit2_label <- paste0("two_signal_kepler_", selected_noise)
fit2 <- export_fit(fit_mcmc(start2, "two_signal"), fit2_label)
fit_rows2 <- fit_rows_for(fit2$best, fit2_label)
phase_plot(fit2$best, fit_rows2, fit2_label,
           file.path(out_dir, paste0("HD361_combined_", fit2_label, "_phase_fit.pdf")))
...
summary <- data.frame(
  ...
  selected_noise_model=selected_noise,
  selected_Nar=Nar,
  selected_Nma=Nma,
